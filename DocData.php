...
<body>
    ...
    <tbody>
            <?php
                $obj=new Credencial();
                $obj->header('Credencial');
                $resultData=$obj->registroDocumentos('credencial','pendente');
                $obj->grab($resultData);

                $obj->header('Anulação de Matrucula');
                $resultData=$obj->registroDocumentos('anulacao','pendente');
                $obj->grab($resultData);

                $obj->header('Equivalêlcia');
                $resultData=$obj->registroDocumentos('equivalencia','pendente');
                $obj->grab($resultData);
                
                $obj->header('Mudaça de Curso');
                $resultData=$obj->registroDocumentos('curso','pendente');
                $obj->grab($resultData);
                
                $obj->header('Notas');
                $resultData=$obj->registroDocumentos('notas','pendente');
                $obj->grab($resultData);
                
                $obj->header('Mudança de Turno');
                $resultData=$obj->registroDocumentos('turno','pendente');
                $obj->grab($resultData);

                $obj->header('Reavaliação de Exame');
                $resultData=$obj->registroDocumentos('realizar_exame','pendente');
                $obj->grab($resultData);
                
                $obj->header('Revisão exame');
                $resultData=$obj->registroDocumentos('revisao_exame','pendente');
                $obj->grab($resultData);    
            ?>
        </tbody>
</div>
